...
function data2txt(name) {
 return typeof window.impData[name] === "string"
  ? window.impData[name]
  : JSON.stringify(window.impData[name], null, 2);
}
...
function render(params) {
 if (!params.name && !params.url) {
  return API.errorNotice(
   "Error",
   "No <strong>name</strong> or <strong>url</strong> specified",
  );
 }
...
function animate(el) {
 ...
 if (el.dataset.isstatic === "true") {
  return;
 }
 const dataname = el.dataset.dataname;
 var databody = window.impData[dataname];
 const filename = el.dataset.filename;
 typeof window.impData[dataname] != "string" &&
  (databody = JSON.stringify(databody, null, 2));
 ...
 el.addEventListener("click", () => {
  ...
  element.setAttribute(
   "href",
   "data:text/plain;charset=utf-8," + encodeURIComponent(databody),
  );
